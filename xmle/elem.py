...
from xml.etree.ElementTree import Element, TreeBuilder, XMLParser, ParseError
...
class Elem(Element):
        ...
        if "matplotlib" in str(type(arg)):
            ...
            if isinstance(arg, matplotlib.figure.Figure):
                try:
                    ...
                    pass
        if pd is not None and isinstance(arg, pd.DataFrame):
            return cls.from_dataframe(arg)
        if pd is not None and isinstance(arg, pd.Series):
            return cls.from_dataframe(pd.DataFrame(arg))
            ...
            if type(err).__name__ == "ParseError":
                if _fix_step == 0:
                    return cls.from_string(
                        xml_as_string.replace("<style scoped>", "<style scoped='1'>"),
                        _fix_step=1,
                    )
                elif _fix_step == 1:
                    return cls.from_string(
                    ...
                    return cls.from_string(
                        xml_as_string.replace("&nbsp;", " "), _fix_step=3
                    )
                else:
                    import logging
                    ...
                    lined = "\n".join(
                        f"{n: 4d} | {line}"
                        for n, line in enumerate(xml_as_string.split("\n"), start=1)
                    )
    ...
    def from_bytes(cls, xml_as_bytes):
        ...
        try:
            return xml.etree.ElementTree.fromstring(
                xml_as_string, parser=XMLParser(target=TreeBuilder(element_factory=cls))
            )
        except Exception as err:  # xml.etree.ElementTree.ParseError
            if type(err).__name__ == "ParseError":
                return cls.from_string(
    ...
    def from_figure(
        ...
        **kwargs,
        ...
        if not isinstance(fig, matplotlib.figure.Figure):
            try:
                ...
                if not hasattr(fig, "savefig"):
                    raise TypeError(
                        "fig must be a figure or provide `get_figure` or `savefig` method."
                    )
        ...
        try:
            ...
            _this_fig = plt.figure(fig_number)  # activate current figure

        existing_format_keys = list(kwargs.keys())
        for key in existing_format_keys:
            if key.upper() != key:
                kwargs[key.upper()] = kwargs[key]
        ...
        if "GRAPHWIDTH" in kwargs and "GRAPHHEIGHT" not in kwargs:
            kwargs["GRAPHHEIGHT"] = kwargs["GRAPHWIDTH"] * 0.67
        imgbuffer = BytesIO()
        fig.savefig(
            imgbuffer,
            dpi=None,
        ...
        try:
            ...
            if callable(arg):
                ...
                if isinstance(arg, bytes) and arg[:5] == b"<svg ":
                    ...
                    s = xml.etree.ElementTree.tostring(
                        arg, encoding="utf8", method="html"
                    ).decode()
    ...
    def is_png(self):
        if len(self) == 1 and self.tag == "div":
            ...
            self.tag == "img"
            and self.attrib["src"][:27] != "data:image/png;base64,iVBOR"
        ):
            return True
        else:
            return False
    ...
    def to_png(self, filename=None):
        ...
        raise TypeError(
            "must be a svg or png element, or a div containing only a svg or png"
        )
    ...
    def from_png_raw(cls, png_raw):
        return cls(
            ...
            src="data:image/png;base64,{}".format(
                base64.standard_b64encode(png_raw).decode()
            ),
        )
    ...
    def __repr__(self):
        return "<xmle.Elem '{}' with {} children>".format(
            self.tag, (len(self))
        )  # +self.pprint()
    ...
    def anchor(self, ref, reftxt, cls, toclevel):
        self.put(
            "a", {"name": ref, "reftxt": reftxt, "class": cls, "toclevel": toclevel}
        )
    ...
    def hn(self, n, content, attrib=None, anchor=None, **extra):
        ...
        if anchor:
            ...
            h_elem.put(
                ...
                {
                    "name": _uid(),
                    "reftxt": anchor if isinstance(anchor, str) else content,
                    "class": "toc",
                    "toclevel": "{}".format(n),
                },
                tail=content,
    ...
    def metadata(self):
        if "metadata" in self.attrib:
            return cloudpickle.loads(
                base64.standard_b64decode(self.attrib["metadata"].encode())
            )
        return None

    @metadata.setter
    def metadata(self, meta):
        self.attrib["metadata"] = base64.standard_b64encode(
